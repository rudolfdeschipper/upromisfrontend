...
export class RequestAPI {
    ...
    static loadList = async (listInfo: IListInfo, token: string) : Promise<ILoadResult<IRequestData>> => 
    {
        try {
            ...
            if(RequestAPI.debug) console.error(ex);
    ...
    static saveRecord = async (message: ISaveMessage<IRequestData>, token: string): Promise<IAPIResult<IRequestData>> => {
        ...
        return await http<ISaveMessage<IRequestData>, IAPIResult<IRequestData>>(
        ...
        .then((result) => {
            if (result.ok && result.parsedBody) {
                if(RequestAPI.debug) console.log("API result ok & parsebody set");
                return { ...result.parsedBody, success: true };
            } else {
                if(RequestAPI.debug) console.log("API result not ok | parsebody not set");
                return { 
                    id: message.id, 
        ...
        }, (rejectedResult: HttpResponse<IAPIResult<IRequestData>>) => {
            if(RequestAPI.debug) console.log("API rejected result not ok");
            return { 
                id: message.id, 
                dataSubject: undefined, 
                success: false, 
        ...
        .catch(ex => {
            if(RequestAPI.debug) console.error("API saveRecord exception:");
            if(RequestAPI.debug) console.log(ex);
            return { 
                id: message.id, 
                dataSubject: undefined, 
                success: false, 
                message: ex.statusText, 
                additionalInfo: (ex as HttpResponse<IAPIResult<IRequestData>>).parsedBody?.additionalInfo
            };
        });
    };
